...
class Handler(SimpleHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args) -> None:
        sys.stderr.write("[playvault] " + (fmt % args) + "\n")
...
def main() -> None:
    if not CONFIG_PATH.exists() and EXAMPLE_PATH.exists():
        ...
        print(f"[playvault] Criado {CONFIG_PATH} a partir do example")
    ...
    print(f"[playvault] UI + API em http://{host}:{port}")
    print(f"[playvault] ROMs em {(ROOT / cfg.get('roms_root', 'roms')).resolve()}")
    print("[playvault] Coloque dumps próprios nas pastas. Ctrl+C para sair.")
    try:
